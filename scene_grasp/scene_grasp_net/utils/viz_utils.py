...
def save_projected_points(color_img, pcd_array, output_path, colors_array=None):
    open_3d_lines = [
        [5, 3],
        [6, 4],
        [0, 2],
    ...
    # Projections
    color = ['g', 'y', 'b', 'r', 'm', 'c', '#3a7c00', '#3a7cd9', '#8b7cd9', '#211249']
    if len(pcd_array) > len(color):
        print("=======>FIXME<======= using same colors for different preds")
    for i, points_2d_mesh in enumerate(pcd_array):
