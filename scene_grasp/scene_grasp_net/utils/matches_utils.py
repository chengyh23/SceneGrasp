...
def get_matches(pred_dp, nocs, out_dir, degree_thresholds=[180], shift_thresholds=[100],
    ...
    iou_pred_matches_all = np.zeros((num_iou_thres, 300), dtype=np.int32)
    iou_pred_scores_all = np.zeros((num_iou_thres, 300))
    iou_gt_matches_all = np.zeros((num_iou_thres, 300), dtype=np.int32)
    iou_pred_count = 0
    iou_gt_count = 0

    pose_pred_matches_all = np.zeros((num_degree_thres, num_shift_thres, 300), dtype=np.int32)
    pose_pred_scores_all = np.zeros((num_degree_thres, num_shift_thres, 300))
    pose_gt_matches_all = np.zeros((num_degree_thres, num_shift_thres, 300), dtype=np.int32)
    pose_pred_count = 0
    pose_gt_count = 0

    # loop over results to gather pred matches and gt matches for iou and pose metrics
    progress = 0
    for progress, pred_dp in enumerate([pred_dp]):
        ...
        for cls_id in range(1, num_classes):
            ...
            pose_pred_matches_all[:, :, pred_start:pred_end] = pose_cls_pred_match
            ...
            pose_pred_scores_all[:, :, pred_start:pred_end] = cls_pred_scores_tile
            num_gt = pose_cls_gt_match.shape[2]
            gt_start = pose_gt_count
            gt_end = gt_start + num_gt
            pose_gt_count = gt_end
            pose_gt_matches_all[:, :, gt_start:gt_end] = pose_cls_gt_match
        # print("===============\n")
